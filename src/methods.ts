...
const { parseMethodRegex, parseConstructorRegex, parseRecordRegex } = (() => {
    ...
    const body = seq(spaceOrLineOptional, cap(neasted("{", "}", 8, false)));

    const method = seq(modifier, async, methodType, methodName, paramList, body);

    const constructorCall = optional(seq(spaceOrLineOptional, /:/, spaceOrLineOptional, any(/base/, /this/), spaceOrLineOptional, neasted("(", ")", 4, false)));
