...
export function parseProperty(code: string): ParseResult<CSharpProperty> | null {
    ...
    if (!match) {
        ...
        var propertyNameWithContextRegex = new RegExp(`((.*(\n|\r|\r\n)){1})[ \t]*${match[1]} ${match[2]} ${match[3]}`);
        if(propertyBlock?.[0] && !propertyBlock[0].includes(match[3])) {
            propertyBlock = undefined
        }
        propertyBlock = propertyBlock || propertyNameWithContextRegex.exec(code)

        return {
            index: match.index,
            length: match[0].length,
            data: {
                isRequired: propertyBlock?.[0] && propertyBlock[0].includes('Required'),
                modifier: match[1],
                type: match[2],
                name: match[3],
