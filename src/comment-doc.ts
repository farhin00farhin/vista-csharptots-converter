    ...
    const text = items.map((x, i): string => {
        ...
        switch (x.type) {
            ...
            case "end":
                return ""
            case "docStart":
                const startingPhrase = "Gets or sets "
                const content = x.content.replace(startingPhrase, "");
                const formattedContent = content.charAt(0).toUpperCase() + content.slice(1)
                return `${x.space}${startChar}${formattedContent}`;
            case "emptyNode":
                return "";
            case "content":
                return x.text;
...
export function parseXmlDocBlock(code: string): ParseResult<XmlDocBlock> | null {
    ...
    const r = ParseRegex<XmlDocBlock>(code, cap(summaryBlockPatt), match => {
        const commentSize = match[1].split(/\r\n|\r|\n/).length;
        var regex = new RegExp(`((.*(\n|\r|\r\n)){${commentSize}})[ \t]*public .*\\(`);
        constructorBlock = constructorBlock || code.match(regex) || regex.exec(code)

        if(constructorBlock?.[0].includes(match[1])) {
            return ({
                items: []
            })
        }
        constructorBlock = undefined;
        return ({
